...
export default function Register({searchParams}: { searchParams: { isError: string, message: string }; }) {
	const signUp = async (formData: FormData) => {
		...
		if (error) {
			...
			return redirect("/auth/register?isError=true&message=Could not authenticate user");
		}

		return redirect("/auth/register?isError=false&message=Check email to continue sign in process");
	};
	...
	return (
		<div className="flex-1 w-full flex flex-col items-center">
			...
			<div className="flex flex-col items-center justify-center gap-5 py-16 w-full max-w-screen-sm p-5">
				...
				<p className={'text-center font-medium max-w-screen-sm'}>Once you have submitted your email, a sign-in
